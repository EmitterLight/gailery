...
Обрати внимание:
- description: описание фотографии. Пиши на русском.
...
Шаблон описания:
[Событие или место]. Кто запечатлён (имена из списка лиц), что делает, во что одет. Окружение и атмосфера.

Примеры:
- [Лаборатория, летний лагерь]. Мария, 13 лет, стоит слева в белой футболке, Ольга, 15 лет, справа в полосатой рубашке. За ними лабораторные столы с колбами.
- [Новогодний праздник]. Мария, 15 лет, слева в праздничном уборе, Ольга, 14 лет, в центре в чёрном платье, справа девушка в длинном чёрном платье. На заднем плане ёлка с игрушками.
- [Прогулка у озера]. Елена стоит у воды в серой футболке и солнцезащитных очках. На переднем плане цветок лотоса.
...
ВОЗРАСТ ДЕТЕЙ:
Если в списке лиц указан возраст — ОБЯЗАТЕЛЬНО включи его рядом с именем.
Пример: "Ольга, 7 лет, стоит справа".
...
def count_undescribed():
    ...
    return db.sqlite.execute(
        ...
        "WHERE (p.description IS NULL OR p.description = '' OR cf.described = 0) AND p.deleted = 0 AND cf.is_canonical = 1 AND (p.media_type IS NULL OR p.media_type != 'video')"
    ).fetchone()[0]
...
WHERE (p.description IS NULL OR p.description = '' OR c.described = 0) AND p.deleted = 0 AND (p.media_type IS NULL OR p.media_type != 'video')
    """
    if dir_filter:
